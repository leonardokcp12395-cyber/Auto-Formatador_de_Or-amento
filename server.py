...
VERSION_MANIFEST_URL = "https://raw.githubusercontent.com/leonardokcp12395-cyber/Auto-Formatador_de_Or-amento/main/version.json"
...
def _validate_http_url(url: str, *, field_name: str = "URL") -> str:
...
def _download_json(url: str) -> Dict[str, Any]:
    ...
    request = urllib.request.Request(
        _validate_http_url(url, field_name="URL do manifesto"),
        headers={
...
def _resolve_update_download_url(request: UpdateRequest) -> str:
    direct_url = (request.url or request.download_url or "").strip()
    manifest_url = (request.manifest_url or "").strip()

    if not manifest_url and direct_url.lower().split("?", 1)[0].endswith(".json"):
        manifest_url = direct_url
        direct_url = ""

    if manifest_url:
        manifest = _download_json(manifest_url)
        # O manifesto novo usa "url" e aponta para o asset correto Planify.exe.
        direct_url = str(manifest.get("url") or manifest.get("download_url") or "").strip()
    ...
    if not direct_url:
        raise HTTPException(status_code=400, detail="Manifesto de atualização não contém a chave 'url'.")

    return _validate_http_url(direct_url, field_name="URL de download")
...
async def trigger_update(request: UpdateRequest):
    ...
    try:
        ...
        download_url = _resolve_update_download_url(request)

        if getattr(sys, "frozen", False):
            executable_path = Path(sys.executable).resolve()
        else:
            executable_path = Path.cwd() / "Planify.exe"

        install_dir = executable_path.parent
        new_executable = install_dir / "Planify_novo.exe"
        ...
        await asyncio.to_thread(urllib.request.urlretrieve, download_url, str(new_executable))
