...
const VERSION_URL = 'https://raw.githubusercontent.com/leonardokcp12395-cyber/Auto-Formatador_de_Or-amento/main/version.json';
...
export const usePlanifyStore = create((set, get) => ({
  ...
  checkForUpdates: async () => {
    ...
    try {
      ...
      const remoteResponse = await fetch(VERSION_URL, {
        cache: 'no-store',
        headers: { Accept: 'application/json' },
      });
      if (!remoteResponse.ok) {
        throw new Error(`Não foi possível consultar o manifesto de atualização (${remoteResponse.status}).`);
      }

      const remoteData = await remoteResponse.json();
      const currentVersion = localData.version || '';
      const latestVersion = remoteData.version || '';
      const updateUrl = remoteData.url || remoteData.download_url || '';
      const updateNotes = remoteData.notes || remoteData.release_notes || remoteData.changelog || '';
  ...
  installUpdate: async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/update`, {
        ...
        body: JSON.stringify({ download_url: updateUrl, manifest_url: VERSION_URL }),
      });
